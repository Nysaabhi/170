  ...
  automotiveDatabase: {
    ...
    "https://cars.tatamotors.com/images/tiago/ice/gallery/exterior/02.jpg"
      ...
      },
      {
        ...
        images: [
        "https://www.royalenfield.com/content/dam/royal-enfield/motorcycles/classic-350/gallery/01.jpg",
    "https://www.royalenfield.com/content/dam/royal-enfield/motorcycles/classic-350/gallery/02.jpg",
    "https://www.royalenfield.com/content/dam/royal-enfield/motorcycles/classic-350/gallery/03.jpg"
        ],
...
class ChatSystem {
  ...
  // Try advanced travel query processing first
  const travelResponse = this.processTravelQuery(normalizedQuery);
  if (travelResponse) return travelResponse;

  // Check for train booking
  const trainResponse = this.processTrainBookingQuery(normalizedQuery);
  if (trainResponse) return trainResponse;

  // Check for flight booking
  const flightResponse = this.processFlightBookingQuery(normalizedQuery);
  if (flightResponse) return flightResponse;

    // Check for bus booking
...
processTravelQuery(query) {
  // Enhanced location patterns
  const locationPattern = /(?:from|departing|departure from)\s+([a-zA-Z\s,]+?)(?=\s+(?:to|and back|return|for|on|in|with|\d|$))/i;
  const destinationPattern = /(?:to|arriving|arrival at)\s+([a-zA-Z\s,]+?)(?=\s+(?:from|and back|return|for|on|in|with|\d|$))/i;

  // Enhanced date patterns (supports relative dates like "next Monday")
  const datePatterns = [
    /(?:on|for|at|date|departing)\s+((?:\d{1,2}(?:st|nd|rd|th)?\s+)?(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+(?:\d{1,2}(?:st|nd|rd|th)?\s+)?\d{4})/i,
    /(?:on|for|at|date|departing)\s+((?:\d{1,2}(?:st|nd|rd|th)?\s+)?(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+(?:\d{1,2}(?:st|nd|rd|th)?)?)/i,
    /(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})/,
  ...
  const returnDatePatterns = [
    /(?:return(?:ing)?|coming back|back)\s+(?:on|at|date)?\s+((?:\d{1,2}(?:st|nd|rd|th)?\s+)?(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+(?:\d{1,2}(?:st|nd|rd|th)?\s+)?\d{4})/i,
    /(?:return(?:ing)?|coming back|back)\s+(?:on|at|date)?\s+((?:\d{1,2}(?:st|nd|rd|th)?\s+)?(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+(?:\d{1,2}(?:st|nd|rd|th)?)?)/i,
    /return\s+(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})/,
    /(?:return(?:ing)?|coming back|back)\s+(?:next|this)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i,
    /(?:return(?:ing)?|coming back|back)\s+(?:in|after)\s+(\d+)\s+(?:days|weeks|months)/i,
    /(?:return(?:ing)?|coming back|back)\s+(?:tomorrow|today)/i
  ];

  // Enhanced passenger patterns
  const passengerPatterns = [
    /(\d+)\s+(?:adult|person|passenger|people|pax)s?/i,
    /(?:for|with)\s+(\d+)\s+(?:adult|person|passenger|people|pax)s?/i,
    /(?:adult|person|passenger|people|pax)s?\s+(\d+)/i
  ];

  // Enhanced class patterns for flights
  const classPattern = /(?:in|by|using|with)\s+(economy|business|first|premium economy)\s+(?:class)?/i;

  // Determine if it's a flight or train query
  const isFlightQuery = /(flight|plane|air|airline|airways|fly)/i.test(query);
  const isTrainQuery = /(train|rail|railway)/i.test(query);

  // Extract information
  const fromMatch = query.match(locationPattern);
  const toMatch = query.match(destinationPattern);
  ...
  let departureDate = 'Not specified';
  for (const pattern of datePatterns) {
    const match = query.match(pattern);
    if (match) {
      departureDate = this.parseAndFormatDate(match[1]);
      break;
    }
  }

  // Extract return date for flights
  let returnDate = null;
  if (isFlightQuery) {
    for (const pattern of returnDatePatterns) {
      const match = query.match(pattern);
      if (match) {
        returnDate = this.parseAndFormatDate(match[1]);
        break;
      }
    }
  }
  ...
  for (const pattern of passengerPatterns) {
    const match = query.match(pattern);
    if (match) {
      passengers = parseInt(match[1]);
      break;
    }
  }

  // Extract travel class for flights
  let travelClass = 'economy';
  if (isFlightQuery) {
    const classMatch = query.match(classPattern);
    if (classMatch) {
      travelClass = classMatch[1].toLowerCase();
    }
  }

  // Create appropriate response
  if (isFlightQuery) {
    const departureFormatted = this.formatDate(departureDate, 'DDMMYY');
    const returnFormatted = returnDate ? this.formatDate(returnDate, 'DDMMYY') : departureFormatted;
    const url = `https://www.skyscanner.co.in/transport/flights/${this.getAirportCode(from)}/${this.getAirportCode(to)}/${departureFormatted}/${returnFormatted}/?adultsv2=${passengers}&cabinclass=${travelClass}&ref=home&rtn=${returnDate ? '1' : '0'}`;
    return this.createFlightBookingResponse(from, to, departureDate, returnDate, passengers, travelClass, url);
  } else if (isTrainQuery) {
    const url = this.isMobileDevice() 
      ? `https://www.confirmtkt.com/rbooking/trains/from/${this.getStationCode(from)}/to/${this.getStationCode(to)}/${this.formatDate(departureDate)}`
      : `https://www.confirmtkt.com/rbooking-d/trains/from/${this.getStationCode(from)}/to/${this.getStationCode(to)}/${this.formatDate(departureDate)}`;
    return this.createTrainBookingResponse(from, to, departureDate, passengers, url);
  }
  ...
  if (dateStr.match(/^\d{1,2}[-/]\d{1,2}[-/]\d{2,4}$/)) {
    ...
    parts.forEach(part => {
      if (part.match(/^\d{1,2}$/)) {
        ...
        year = parseInt(part);
      } else if (months.hasOwnProperty(part)) {
        month = months[part];
      }
    });
...
getAirportCode(city) {
  // Simplified airport code mapping
  const airports = {
    'mumbai': 'BOM',
    'delhi': 'DEL',
    'bangalore': 'BLR',
  ...
  return airports[city.toLowerCase()] || city.toUpperCase().slice(0, 3);
}

getStationCode(city) {
  // Simplified station code mapping
  const stations = {
    'mumbai': 'CSMT',
    'delhi': 'NDLS',
    'bangalore': 'SBC',
  ...
  return stations[city.toLowerCase()] || city.toUpperCase();
}

getCityId(city) {
  // Simplified city ID mapping
  const cities = {
    'mumbai': 1,
    'delhi': 733,
    'bangalore': 3,
...
processBusQuery(query) {
  ...
  // Generate redBus URL
  const url = `https://www.redbus.in/bus-tickets/${from.toLowerCase()}-to-${to.toLowerCase()}?fromCityName=${from}&fromCityId=${this.getCityId(from)}&srcCountry=IND&toCityName=${to}&toCityId=${this.getCityId(to)}&destCountry=IND&onward=${this.formatDate(departureDate, 'DD-MMM-YYYY')}&opId=0&busType=Any`;

  return this.createBusBookingResponse(from, to, departureDate, passengers, url);
}

processTrainBookingQuery(query) {
  const trainPattern = /(?:book|find|reserve|get) (?:a )?train (?:from )?(\w+)(?: to )?(\w+)(?: (?:on|for) (\d{1,2}(?:st|nd|rd|th)? \w+))?(?: for (\d+) (?:adult|passenger)s?)?/i;
  const match = query.match(trainPattern);
  if (match) {
    const from = match[1].toUpperCase();
    const to = match[2].toUpperCase();
    const date = match[3] ? this.formatDate(match[3]) : 'Not specified';
    const passengers = match[4] || 1;
    const url = `https://www.confirmtkt.com/rbooking/trains/from/${from}/to/${to}/${date}`;
    return this.createTrainBookingResponse(from, to, date, passengers, url);
  }
  return null;
}

processFlightBookingQuery(query) {
  const flightPattern = /(?:book|find|reserve|get) (?:a )?flight (?:from )?(\w+)(?: to )?(\w+)(?: (?:on|for) (\d{1,2}(?:st|nd|rd|th)? \w+))?(?: and return on (\d{1,2}(?:st|nd|rd|th)? \w+))?(?: for (\d+) (?:adult|passenger)s?)?(?: in (\w+) class)?/i;
  const match = query.match(flightPattern);
  if (match) {
    const from = match[1].toUpperCase();
    const to = match[2].toUpperCase();
    const departureDate = match[3] ? this.formatDate(match[3], 'DDMMYY') : 'Not specified';
    const returnDate = match[4] ? this.formatDate(match[4], 'DDMMYY') : departureDate;
    const passengers = match[5] || 1;
    const travelClass = match[6] || 'economy';
    const url = `https://www.skyscanner.co.in/transport/flights/${from}/${to}/${departureDate}/${returnDate}/?adultsv2=${passengers}&cabinclass=${travelClass}&ref=home&rtn=1`;
    return this.createFlightBookingResponse(from, to, departureDate, returnDate, passengers, travelClass, url);
  }
  return null;
}
  ...
  // Enhanced stock pattern to capture various ways users might ask for stock prices
  const stockPattern = /(?:check|view|show|what is|what's|price of| stock price of|share price of)\s*(?:the)?\s*([\w\s-]+)\s*(?:stock|share|price)?/i;
  const match = query.match(stockPattern);
  if (match) {
    const stock = match[1].trim().toUpperCase();
    
    // Map of company names to their respective URLs
    const stockUrls = {
  // NSE Stocks
  ...
  'HDFC': 'https://upstox.com/stocks/housing-development-finance-corp-ltd-share-price/',
  'INFOSYS': 'https://upstox.com/stocks/infosys-ltd-share-price/',
  'WIPRO': 'https://upstox.com/stocks/wipro-ltd-share-price/',
  'TATA MOTORS': 'https://upstox.com/stocks/tata-motors-ltd-share-price/',
  'AXIS BANK': 'https://upstox.com/stocks/axis-bank-ltd-share-price/',
  'ITC': 'https://upstox.com/stocks/itc-ltd-share-price/',
  'SBI': 'https://upstox.com/stocks/state-bank-of-india-share-price/',
  'HINDALCO': 'https://upstox.com/stocks/hindalco-industries-ltd-share-price/',
  'ICICI BANK': 'https://upstox.com/stocks/icici-bank-ltd-share-price/',
  'BHARTI AIRTEL': 'https://upstox.com/stocks/bharti-airtel-ltd-share-price/',
  'LT': 'https://upstox.com/stocks/larsen-toubro-ltd-share-price/',
  'KOTAK MAHINDRA BANK': 'https://upstox.com/stocks/kotak-mahindra-bank-ltd-share-price/',
  'HCL TECH': 'https://upstox.com/stocks/hcl-technologies-ltd-share-price/',
  'ASIAN PAINTS': 'https://upstox.com/stocks/asian-paints-ltd-share-price/',
  'MARUTI SUZUKI': 'https://upstox.com/stocks/maruti-suzuki-india-ltd-share-price/',
  'ULTRATECH CEMENT': 'https://upstox.com/stocks/ultratech-cement-ltd-share-price/',
  'NTPC': 'https://upstox.com/stocks/ntpc-ltd-share-price/',
  'ONGC': 'https://upstox.com/stocks/oil-natural-gas-corporation-ltd-share-price/',
  'POWERGRID': 'https://upstox.com/stocks/power-grid-corporation-of-india-ltd-share-price/',
  'NESTLE': 'https://upstox.com/stocks/nestle-india-ltd-share-price/',
  'BAJAJ AUTO': 'https://upstox.com/stocks/bajaj-auto-ltd-share-price/',
  'TATA STEEL': 'https://upstox.com/stocks/tata-steel-ltd-share-price/',
  'SUN PHARMA': 'https://upstox.com/stocks/sun-pharmaceutical-ind-ltd-share-price/',
  'ADANI PORTS': 'https://upstox.com/stocks/adani-ports-and-sez-ltd-share-price/',
  'JSW STEEL': 'https://upstox.com/stocks/jsw-steel-ltd-share-price/',
  'DR. REDDYS': 'https://upstox.com/stocks/dr-reddys-laboratories-ltd-share-price/',
  'BAJAJ FINANCE': 'https://upstox.com/stocks/bajaj-finance-ltd-share-price/',
  'GRASIM': 'https://upstox.com/stocks/grasim-industries-ltd-share-price/',
  'INDUSIND BANK': 'https://upstox.com/stocks/indusind-bank-ltd-share-price/',
  'COAL INDIA': 'https://upstox.com/stocks/coal-india-ltd-share-price/',
  'TECH MAHINDRA': 'https://upstox.com/stocks/tech-mahindra-ltd-share-price/',
  'TITAN': 'https://upstox.com/stocks/titan-company-ltd-share-price/',
  'CIPLA': 'https://upstox.com/stocks/cipla-ltd-share-price/',
  'HERO MOTOCORP': 'https://upstox.com/stocks/hero-motocorp-ltd-share-price/',
  'EICHER MOTORS': 'https://upstox.com/stocks/eicher-motors-ltd-share-price/',
  'BPCL': 'https://upstox.com/stocks/bharat-petroleum-corporation-ltd-share-price/',
  'GAIL': 'https://upstox.com/stocks/gail-india-ltd-share-price/',
  'M&M': 'https://upstox.com/stocks/mahindra-mahindra-ltd-share-price/',
  'VEDANTA': 'https://upstox.com/stocks/vedanta-ltd-share-price/',
  'SHREE CEMENT': 'https://upstox.com/stocks/shree-cement-ltd-share-price/',
  'UPL': 'https://upstox.com/stocks/upl-ltd-share-price/',
  'DIVIS LAB': 'https://upstox.com/stocks/divis-laboratories-ltd-share-price/',
  'IOC': 'https://upstox.com/stocks/indian-oil-corporation-ltd-share-price/',
  'BAJAJ FINSERV': 'https://upstox.com/stocks/bajaj-finserv-ltd-share-price/',
  'ZEE ENTERTAINMENT': 'https://upstox.com/stocks/zee-entertainment-enterprises-ltd-share-price/',
  'BRITANNIA': 'https://upstox.com/stocks/britannia-industries-ltd-share-price/',
  'HDFC LIFE': 'https://upstox.com/stocks/hdfc-life-insurance-company-ltd-share-price/',
  'HINDUSTAN UNILEVER': 'https://upstox.com/stocks/hindustan-unilever-ltd-share-price/',
  'SBIN': 'https://upstox.com/stocks/state-bank-of-india-share-price/',
  'TATA CONSUMER': 'https://upstox.com/stocks/tata-consumer-products-ltd-share-price/',
  'ADANI ENTERPRISES': 'https://upstox.com/stocks/adani-enterprises-ltd-share-price/',
  'ADANI GREEN': 'https://upstox.com/stocks/adani-green-energy-ltd-share-price/',
  'ADANI TRANSMISSION': 'https://upstox.com/stocks/adani-transmission-ltd-share-price/',
  'ADANI TOTAL GAS': 'https://upstox.com/stocks/adani-total-gas-ltd-share-price/',
  'ADANI PORTS': 'https://upstox.com/stocks/adani-ports-and-sez-ltd-share-price/',
  'ADANI POWER': 'https://upstox.com/stocks/adani-power-ltd-share-price/',
  'ADANI WILMAR': 'https://upstox.com/stocks/adani-wilmar-ltd-share-price/',
  ...
  'BERKSHIRE HATHAWAY': 'https://finance.yahoo.com/quote/BRK-A/',
  ...
  'PROCTER & GAMBLE': 'https://finance.yahoo.com/quote/PG/',
  'UNITEDHEALTH': 'https://finance.yahoo.com/quote/UNH/',
  'HOME DEPOT': 'https://finance.yahoo.com/quote/HD/',
  ...
  'PEPSICO': 'https://finance.yahoo.com/quote/PEP/',
  'NETFLIX': 'https://finance.yahoo.com/quote/NFLX/',
  'ADOBE': 'https://finance.yahoo.com/quote/ADBE/',
  'PAYPAL': 'https://finance.yahoo.com/quote/PYPL/',
  'INTEL': 'https://finance.yahoo.com/quote/INTC/',
  'CISCO': 'https://finance.yahoo.com/quote/CSCO/',
  'VERIZON': 'https://finance.yahoo.com/quote/VZ/',
  'AT&T': 'https://finance.yahoo.com/quote/T/',
  'DISNEY': 'https://finance.yahoo.com/quote/DIS/',
  'MCDONALDS': 'https://finance.yahoo.com/quote/MCD/',
  'NIKE': 'https://finance.yahoo.com/quote/NKE/',
  'BOEING': 'https://finance.yahoo.com/quote/BA/',
  'GOLDMAN SACHS': 'https://finance.yahoo.com/quote/GS/',
  'MORGAN STANLEY': 'https://finance.yahoo.com/quote/MS/',
  'EXXON MOBIL': 'https://finance.yahoo.com/quote/XOM/',
  'CHEVRON': 'https://finance.yahoo.com/quote/CVX/',
  'PFIZER': 'https://finance.yahoo.com/quote/PFE/',
  'MODERNA': 'https://finance.yahoo.com/quote/MRNA/',
  'ALIBABA': 'https://finance.yahoo.com/quote/BABA/',
  'TENCENT': 'https://finance.yahoo.com/quote/TCEHY/',
  'SAMSUNG': 'https://finance.yahoo.com/quote/SSNLF/',
  'TOYOTA': 'https://finance.yahoo.com/quote/TM/',
  'SONY': 'https://finance.yahoo.com/quote/SONY/',
  // Add more US stocks and their URLs as needed
};

    // Get the URL for the queried stock
    const stockUrl = stockUrls[stock];
  ...
  createBusBookingResponse(from, to, date, passengers, url) {
  return `
    <div class="bus-booking-response">
      <p>🚌 Bus Booking Details:</p>
      <ul>
        <li>From: ${from}</li>
        <li>To: ${to}</li>
        <li>Date: ${date}</li>
      ...
      <button class="click-button" data-url="${url}">
        <i class="fas fa-bus"></i> Check Buses
      </button>
    </div>
  `;
}

  createTrainBookingResponse(from, to, date, passengers, url) {
  return `
    <div class="train-booking-response">
      <p>🚆 Train Booking Details:</p>
      <ul>
        <li>From: ${from}</li>
        <li>To: ${to}</li>
        <li>Date: ${date}</li>
      ...
      <button class="click-button" data-url="${url}">
        <i class="fas fa-train"></i> Check Trains
      </button>
    </div>
  `;
}

createFlightBookingResponse(from, to, departureDate, returnDate, passengers, travelClass, url) {
  return `
    <div class="flight-booking-response">
      <p>✈️ Flight Booking Details:</p>
      <ul>
        <li>From: ${from}</li>
        <li>To: ${to}</li>
        <li>Departure: ${departureDate}</li>
        ${returnDate !== departureDate ? `<li>Return: ${returnDate}</li>` : ''}
        <li>Passengers: ${passengers}</li>
        <li>Class: ${travelClass}</li>
      </ul>
      <button class="click-button" data-url="${url}">
        <i class="fas fa-plane"></i> Check Flights
      </button>
    </div>
  `;
}
  ...
  // If the URL is not available, show a disabled button
